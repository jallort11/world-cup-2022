...
This workspace specifies and delivers a **World Cup 2022 dashboard**: a browser UI that visualizes the existing FastAPI endpoints, plus automated tests for those features.
...
2. **Testing:** Every approved `FR` and `NFR` must have at least one verification item in `04-verification.md`. A requirement is done only when its evidence is recorded. Automated tests (pytest) are the default evidence for API and dashboard-serving behavior.
...
5. **Constraints:** Work from approved requirements and design. Link decisions, tasks, and tests to requirement IDs. English is the language of specification, UI copy, and code comments. Stack: FastAPI, pandas, static HTML/CSS/JS served by the same app, pytest. No new frontend framework (React/Vue) and no new backend framework.
...
- Q-002: Mixed — existing JSON API + new HTML dashboard + tests.
- Q-003: FastAPI + pandas + static HTML/CSS/JS + pytest. No React/Vue. No auth.
